...
void main() {
 	vec2 uv = texCoord;
  	...
  	color.r = texture(tex, uv + vec2(offset(16.0, uv) * 0.03, 0.0)).r;
  	color.g = texture(tex, uv + vec2(offset(16.0, uv) * 0.03 * 0.0, 0.0)).g;
  	color.b = texture(tex, uv + vec2(offset(16.0, uv) * 0.03, 0.0)).b;
